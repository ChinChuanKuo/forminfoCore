...
namespace forminfoCore.Models
{
    public class ProformClass
    {
            ...
            foreach (DataRow dr in database.checkSelectSql("mssql", "flyformstring", "exec web.searchclisubform @formId,@inoper;", dbparamlist).Rows)
            {
                ...
                switch (dr["outValue"].ToString().TrimEnd())
                {
                    ...
                    case "checkbox":
                        ...
                        {
                            answeritems.Add(new Dictionary<string, object>() { { "id", int.Parse(drs["id"].ToString().TrimEnd()) }, { "value", drs["value"].ToString().TrimEnd() }, { "showAnswer", drs["answer"].ToString().TrimEnd() == "1" }, { "showDelete", false }, { "showModify", false }, { "showCreate", false } });
                        }
                        break;
                }
        ...
        public statusModels GetInsertModels(uFormData uFormData, string cuurip)
        {
            database database = new database();
            DataTable mainRows = new DataTable();
            List<dbparam> dbparamlist = new List<dbparam>();
            dbparamlist.Add(new dbparam("@formId", uFormData.formId.TrimEnd()));
            dbparamlist.Add(new dbparam("@inoper", uFormData.newid.TrimEnd()));
            mainRows = database.checkSelectSql("mssql", "flyformstring", "exec web.searchclimainform @formId,@inoper;", dbparamlist);
            switch (mainRows.Rows.Count)
            {
                case 0:
            ...
            foreach (var item in uFormData.items)
            {
                switch (bool.Parse(item["itemCreate"].ToString().TrimEnd()))
                {
                    case true:
                        dbparamlist.Clear();
                        dbparamlist.Add(new dbparam("@formId", uFormData.formId.TrimEnd()));
                        ...
                        break;
                    default:
                        switch (bool.Parse(item["itemDelete"].ToString().TrimEnd()))
                        {
                            case true:
                                dbparamlist.Clear();
                                dbparamlist.Add(new dbparam("@formId", uFormData.formId.TrimEnd()));
                                ...
                                break;
                            default:
                                switch (bool.Parse(item["itemModify"].ToString().TrimEnd()))
                                {
                                    case true:
                                        dbparamlist.Clear();
                                        dbparamlist.Add(new dbparam("@tile", item["title"].ToString().TrimEnd()));
                                        ...
                                        dbparamlist.Add(new dbparam("@mooper", uFormData.newid.TrimEnd()));
                                        dbparamlist.Add(new dbparam("@formId", uFormData.formId.TrimEnd()));
                                        dbparamlist.Add(new dbparam("@iid", int.Parse(item["iid"].ToString().TrimEnd())));
                                        dbparamlist.Add(new dbparam("@inoper", uFormData.newid.TrimEnd()));
                                ...
                                foreach (var answeritem in JsonSerializer.Deserialize<List<Dictionary<string, object>>>(item["answeritems"].ToString().TrimEnd()))
                                {
                                    switch (bool.Parse(answeritem["showDelete"].ToString().TrimEnd()))
                                    {
                                        case true:
                                            ...
                                            break;
                                        default:
                                            switch (bool.Parse(answeritem["showCreate"].ToString().TrimEnd()))
                                            {
                                                case true:
                                                    dbparamlist.Clear();
                                                    dbparamlist.Add(new dbparam("@formId", uFormData.formId.TrimEnd()));
                                                    ...
                                                    break;
                                                default:
                                                    switch (bool.Parse(answeritem["showModify"].ToString().TrimEnd()))
                                                    {
                                                        case true:
                                                            dbparamlist.Clear();
                                                            dbparamlist.Add(new dbparam("@value", answeritem["value"].ToString().TrimEnd()));
            ...
            foreach (var settitem in uFormData.settitems)
            {
                dbparamlist.Clear();
                dbparamlist.Add(new dbparam("@tile", uFormData.tile.TrimEnd()));
                dbparamlist.Add(new dbparam("@desc", uFormData.desc.TrimEnd()));
